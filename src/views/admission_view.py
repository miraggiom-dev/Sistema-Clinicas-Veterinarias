...
class AdmissionView(ctk.CTkFrame):
    ...
    def crear_tab_propietarios(self, tab):
        ...
        self.entry_busqueda = ctk.CTkEntry(
            frame_busqueda, placeholder_text="Buscar por nombre o cédula...", width=250
        )
        self.entry_busqueda.pack(side="left", padx=10, pady=10)

        btn_buscar = ctk.CTkButton(
            frame_busqueda, text="Buscar", command=self.actualizar_lista_propietarios
        )
        btn_buscar.pack(side="left", padx=10, pady=10)

        btn_nuevo = ctk.CTkButton(
            frame_busqueda,
            text="Registrar Nuevo Propietario",
            command=self.mostrar_formulario_propietario,
        )
        btn_nuevo.pack(side="right", padx=10, pady=10)

        self.listbox_propietarios = ctk.CTkScrollableFrame(
            tab, label_text="Lista de Clientes Encontrados"
        )
    ...
    def actualizar_lista_propietarios(self):
        ...
        for prop in propietarios:
            ...
            ctk.CTkLabel(frame_item, text=info_text, anchor="w").pack(
                side="left", padx=10
            )
    ...
    def mostrar_formulario_propietario(self):
        ...
        ctk.CTkLabel(win, text="Registrar Cliente", font=("Roboto", 18, "bold")).pack(
            pady=10
        )

        entry_cedula = ctk.CTkEntry(
            win, placeholder_text="Cédula/Documento de Identidad", width=300
        )
        ...
        def guardar_propietario():
            ...
            if id_prop:
                lbl_status.configure(
                    text=f"Cliente registrado con Cédula: {cedula}", text_color="green"
                )
                self.actualizar_lista_propietarios()
                win.after(1500, win.destroy)
            else:
                lbl_status.configure(
                    text="Error al registrar cliente. (Verifique que la Cédula no exista)",
                    text_color="red",
                )

        ctk.CTkButton(win, text="Guardar Cliente", command=guardar_propietario).pack(
            pady=10
        )
    ...
    def mostrar_modal_editar_propietario(self, propietario):
        ...
        ctk.CTkLabel(win, text="Editar Cliente", font=("Roboto", 18, "bold")).pack(
            pady=10
        )
        ...
        def guardar_cambios():
            ...
            result = self.admission_controller.actualizar_cliente(
                (
                    prop.get("id_propietario")
                    if isinstance(prop, dict)
                    else propietario["id_propietario"]
                ),
                nombre=nombre,
                telefono=telefono,
                email=email,
            ...
            if success:
                lbl_status.configure(
                    text=message or "Datos actualizados.", text_color="green"
                )
                self.actualizar_lista_propietarios()
                win.after(1200, win.destroy)
            else:
                lbl_status.configure(
                    text=message or "Error al actualizar. Verifique los datos.",
                    text_color="red",
                )

        ctk.CTkButton(win, text="Guardar cambios", command=guardar_cambios).pack(
            pady=10
        )
    ...
    def crear_tab_mascotas(self, tab):
        ...
        self.lbl_info_prop = ctk.CTkLabel(
            self.frame_info_propietario,
            text="Seleccione un propietario primero.",
            text_color="orange",
        )
        self.lbl_info_prop.pack(pady=10)

        self.btn_nueva_mascota = ctk.CTkButton(
            self.frame_info_propietario,
            text="Registrar Nueva Mascota",
            state="disabled",
            command=self.mostrar_formulario_mascota,
        )
        self.btn_nueva_mascota.pack(side="right", padx=10)

        self.listbox_mascotas = ctk.CTkScrollableFrame(
            tab, label_text="Mascotas del Cliente"
        )
    ...
    def actualizar_tab_mascotas(self):
        ...
        if not self.propietario_seleccionado:
            self.lbl_info_prop.configure(
                text="Seleccione un propietario en la pestaña anterior para ver sus mascotas.",
                text_color="orange",
            )
        ...
        self.lbl_info_prop.configure(
            text=f"Propietario Actual: {nombre_prop} (Cédula: {cedula_prop})",
            text_color="white",
        )
        self.btn_nueva_mascota.configure(state="normal")

        mascotas = self.admission_controller.obtener_mascotas_cliente(
            self.propietario_seleccionado["id_propietario"]
        )

        if not mascotas:
            ctk.CTkLabel(
                self.listbox_mascotas,
                text=f"'{nombre_prop}' no tiene mascotas registradas.",
                text_color="gray",
            ).pack(pady=20)
            return
        ...
        for masc in mascotas:
            ...
            info_text = (
                f"Nombre: {masc['nombre']} ({masc['especie']} | Raza: {masc['raza']})"
            )
            ctk.CTkLabel(frame_item, text=info_text, anchor="w").pack(
                side="left", padx=10
            )
    ...
    def ir_agendar(self, mascota):
        ...
        self.switch_callback(
            "AppointmentView", id_mascota=mascota["id_mascota"], active_tab="Agendar"
        )
    ...
    def mostrar_formulario_mascota(self):
        ...
        ctk.CTkLabel(
            win,
            text=f"Mascota para: {self.propietario_seleccionado['nombre']}",
            font=("Roboto", 16, "bold"),
        ).pack(pady=10)
        ...
        entry_especie = ctk.CTkEntry(
            win, placeholder_text="Especie (Perro, Gato, etc.)", width=300
        )
        ...
        entry_nacimiento = ctk.CTkEntry(
            win, placeholder_text="Fecha de Nacimiento (YYYY-MM-DD)", width=300
        )
        entry_nacimiento.pack(pady=5)

        genero_var = ctk.StringVar(value="Macho")
        ctk.CTkOptionMenu(win, values=["Macho", "Hembra"], variable=genero_var).pack(
            pady=5
        )
        ...
        def guardar_mascota():
            ...
            if exito:
                lbl_status.configure(
                    text=f"Mascota '{entry_nombre.get()}' registrada.",
                    text_color="green",
                )
                self.actualizar_tab_mascotas()
                win.after(1500, win.destroy)
                ...
                lbl_status.configure(
                    text="Error al registrar mascota.", text_color="red"
                )

        ctk.CTkButton(win, text="Guardar Mascota", command=guardar_mascota).pack(
            pady=10
        )
