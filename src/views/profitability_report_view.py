...
from controllers.profitability_report_controller import ProfitabilityReportController
import os
import webbrowser
...
class ProfitabilityReportView(ctk.CTkToplevel):
    ...
    def __init__(self, master=None):
        ...
        self.lbl_status = ctk.CTkLabel(container, text="", text_color="green", wraplength=400)
    ...
    def _generar(self):
        ...
        ruta = ProfitabilityReportController.generar_informe(mes_int, anio_int, tipo)
        ...
        if ruta:
            ...
            self.lbl_status.configure(text=f"No hay datos para {tipo} en este periodo.", text_color="orange")

    def _abrir_carpeta(self):
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        base = os.path.join(project_root, 'informes_rentabilidad')
        try:
